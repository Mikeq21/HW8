...
class ProblemSolutions {
    ...
    public boolean canFinish(int numExams,
        ...
        int[] inDegree = new int[numExams]; // Tarck number of incoming edges
        ...
        // Enqueue nodes with no prerequisites (in-degree 0)
        ...
        while (!queue.isEmpty()) {
            int node = queue.poll();
            count++;  // One more exam can be taken
        ...
        return count == numExams; // All exams can be taken if count == numExams
    } // CanFinish Method - Michael Quiroga
    ...
    public int numGroups(int[][] adjMatrix) {
        ...
        return groups;
    } //numGroups method - Michael Quiroga

    private void dfsGraph(int node, Map<Integer, List<Integer>> graph, boolean[] visited) {
        visited[node] = true;
    ...
    } // dfsGraph HELPER METHOD - Michael Quiroga
}
